...
def allocate_fields():
    ...
    ti.root.place(loss, x_avg, kinetic)
...
@ti.kernel
def compute_kinetic(total_steps:ti.i32):
    for t in range(total_steps-1):
       for i in range(n_particles):
            contrib = 0.0
            if particle_type[i] == 1:
                contrib = 1.0 / n_solid_particles
            kinetic[None].atomic_add(contrib*0.5*(v[t,i][0]**2 + v[t, i][1]**2+x[t,i][1]))


@ti.kernel
def compute_loss():
    dist = x_avg[None][0]
    loss[None] = -dist + kinetic[None]/500.0
    kinetic[None] = 0
...
def worm(scene):
    scene.set_offset(0.1, 0.03)
#    scene.add_rect(0.0, 0.1, 0.3, 0.1, -1)
    scene.add_rect(0.0, 0.0, 0.1, 0.1, 0)
    scene.add_rect(0.0, 0.1, 0.1, 0.1, 1)
    scene.add_rect(0.1, 0.0, 0.1, 0.1, 2)
    scene.add_rect(0.1, 0.1, 0.1, 0.1, 3)
    scene.add_rect(0.2, 0.0, 0.1, 0.1, 4)
    scene.add_rect(0.2, 0.1, 0.1, 0.1, 5)
    scene.add_rect(0.3, 0.0, 0.1, 0.1, 6)
    scene.add_rect(0.3, 0.1, 0.1, 0.1, 7)
    ...
    for iter in range(20001):
        ...
        if iter % 1000 == 0:
            ...
            for s in range(15, 1500, 16):
                visualize(s, 'diffmpm/iter{:05d}/'.format(iter))
